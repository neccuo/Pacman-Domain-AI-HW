...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #print "pacmanPos:" + str(pacmanPosition)
        #print self.beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        legalPoss = self.legalPositions
        oldBeliefs = self.getBeliefDistribution()
        newBeliefs = util.Counter()

        for oldPos in legalPoss:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                #print "oldPos: " + str(oldPos)
                #print "newPos: " + str(newPos)
                #print "prob: " + str(prob)
                newBeliefs[newPos] += prob * oldBeliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        eachPos = particleCount // len(legal)
        rem = particleCount % len(legal)
        if rem == 0:
            for pos in legal:
                for i in range(eachPos):
    ...
    def observe(self, observation, gameState):
        ...
        if not self.isWeighted(allPossible):
            self.initializeUniformly(gameState)
        else:
            newParticles = []
            for i in range(pCount):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in oldParticles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            samp = util.sample(newPosDist)
            newParticles.append(samp)
